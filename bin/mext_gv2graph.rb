...
def usage
  puts "mext2graph.rb --file=<FILE> [OPTIONS]
...
File.open(File.expand_path("command_counters.csv", opt[:dest]), "w") do |f|
  ...
  for i in 1..(mysqladmin["Com_select"].length-2)
...
if mysqladmin["Innodb_adaptive_hash_hash_searches"]
  File.open(File.expand_path("innodb_adaptive_hash_searches.csv", opt[:dest]), "w") do |f|
    ...
    for i in 1..(mysqladmin["Innodb_adaptive_hash_hash_searches"].length-2)
...
if mysqladmin["Innodb_ibuf_merges"]
  File.open(File.expand_path("innodb_insert_buffer.csv", opt[:dest]), "w") do |f|
    ...
    for i in 1..(mysqladmin["Innodb_ibuf_merges"].length-2)
...
File.open(File.expand_path("myisam_indexes.csv", opt[:dest]), "w") do |f|
  ...
  for i in 1..(mysqladmin["Key_read_requests"].length-2)
...
File.open(File.expand_path("temp_tables.csv", opt[:dest]), "w") do |f|
  ...
  for i in 1..(mysqladmin["Created_tmp_tables"].length-2)
...
if mysqladmin["Innodb_checkpoint_age"]
  File.open(File.expand_path("innodb_checkpoint_age.csv", opt[:dest]), "w") do |f|
    ...
    for i in 1..(mysqladmin["Innodb_checkpoint_age"].length-2)
...
File.open(File.expand_path("innodb_row_lock_waits.csv", opt[:dest]), "w") do |f|
  ...
  for i in 1..(mysqladmin["Innodb_row_lock_waits"].length-2)
...
File.open(File.expand_path("innodb_row_lock_time.csv", opt[:dest]), "w") do |f|
  ...
  for i in 1..(mysqladmin["Innodb_row_lock_time"].length-2)
...
if innodb_adaptive_hash_searches
  ...
  `Rscript --no-save #{File.dirname(__FILE__)}/nondiff_innodb_adaptive_hash_searches.R #{File.expand_path("innodb_adaptive_hash_searches.csv", opt[:dest])} #{File.expand_path("innodb_adaptive_hash_searches.png", opt[:dest])}`
else
...
puts "#{`date`.chomp} - Running nondiff_innodb_buffer_pool_activity.R" if opt[:debug]
`Rscript --no-save #{File.dirname(__FILE__)}/nondiff_innodb_buffer_pool_activity.R #{File.expand_path("innodb_buffer_pool_activity.csv", opt[:dest])} #{File.expand_path("innodb_buffer_pool_activity.png", opt[:dest])}`

puts "#{`date`.chomp} - Running nondiff_select_types.R" if opt[:debug]
`Rscript --no-save #{File.dirname(__FILE__)}/nondiff_select_types.R #{File.expand_path("select_types.csv", opt[:dest])} #{File.expand_path("select_types.png", opt[:dest])}`
puts "#{`date`.chomp} - Running nondiff_sorts.R" if opt[:debug]
`Rscript --no-save #{File.dirname(__FILE__)}/nondiff_sorts.R #{File.expand_path("sorts.csv", opt[:dest])} #{File.expand_path("sorts.png", opt[:dest])}`
puts "#{`date`.chomp} - Running nondiff_transaction_handlers.R" if opt[:debug]
`Rscript --no-save #{File.dirname(__FILE__)}/nondiff_transaction_handlers.R #{File.expand_path("transaction_handlers.csv", opt[:dest])} #{File.expand_path("transaction_handlers.png", opt[:dest])}`

if innodb_ibuf
  puts "#{`date`.chomp} - Running nondiff_innodb_insert_buffer.R" if opt[:debug]
  `Rscript --no-save #{File.dirname(__FILE__)}/nondiff_innodb_insert_buffer.R #{File.expand_path("innodb_insert_buffer.csv", opt[:dest])} #{File.expand_path("innodb_insert_buffer.png", opt[:dest])}`
else
...
puts "#{`date`.chomp} - Running nondiff_temp_tables.R" if opt[:debug]
`Rscript --no-save #{File.dirname(__FILE__)}/nondiff_temp_tables.R #{File.expand_path("temp_tables.csv", opt[:dest])} #{File.expand_path("temp_tables.png", opt[:dest])}`
...
puts "#{`date`.chomp} - Running nondiff_innodb_row_lock_waits.R" if opt[:debug]
`Rscript --no-save #{File.dirname(__FILE__)}/nondiff_innodb_row_lock_waits.R #{File.expand_path("innodb_row_lock_waits.csv", opt[:dest])} #{File.expand_path("innodb_row_lock_waits.png", opt[:dest])}`
...
File.open(File.expand_path("mext_gv.html", opt[:dest]), "w") do |f|
  ...
  f.write("<br /><img src='handlers.png' />")

  if innodb_adaptive_hash_searches
    f.write("<br /><img src='innodb_adaptive_hash_searches.png' />")
  end

  f.write("<br /><img src='innodb_buffer_pool.png' />")
  f.write("<br /><img src='innodb_buffer_pool_activity.png' />")
  f.write("<br /><img src='select_types.png' />")

  f.write("<br /><img src='sorts.png' />")
  f.write("<br /><img src='transaction_handlers.png' />")

  if innodb_ibuf
    f.write("<br /><img src='innodb_insert_buffer.png' />")
  end
  ...
  if mysqladmin["Innodb_checkpoint_age"]
    f.write("<br /><img src='innodb_checkpoint_age.png' />")
  end
